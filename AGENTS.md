# Active checkpoint — Phase 8 public-content source complete

The owner authorized the next phase and GitHub publication; read docs/decisions/2026-09-23-phase-8-execution.md. Canonical master: docs/redesign/RivyaLivingArt-Commercial-Implementation-Plan.md revision 3.9. Read PHASE-8-PUBLIC-CONTENT.md and IMPLEMENTATION_PROGRESS.md. P8.1–P8.7 source checkpoint is complete; P8.4 undocumented operating-policy particulars remain a publication gate. Next independent task: P9.1. No repeat plan or asset approval.

Phase 8 reviews all 47 documents/36 articles, corrects 25 source documents, provides 12 FAQs, contextual public/editorial navigation, truthful portfolio empties, factual current-practice policies, published-only content/media projection, aliases/SEO/structured data, route/form dispositions and loading/recovery states. Studio can compare/apply reviewed copy deliberately; source changes never overwrite saved drafts or published revisions automatically. All original IDs/routes/section identities, products, approved image bytes and the historical candidate are preserved.

Branch codex/phase-8-public-content starts from main 630521de2f228d2fe935585e12e4639e98da8839, PR #22. Verify fresh refs and phase-8-github-publication.json before continuing. No database read/write, schema migration, content publication, reference operation or deployment occurred. Formal QA remains Phase 11; compiler/source checks are not runtime proof.

Future Preview and Production share the modern database and private reference store by explicit owner approval. Existing deployments keep captured settings. Read SHARED-DATA-CONFIGURATION.md; old integration associations can overwrite aligned variables. Synthetic QA/full restore requires DIFFERENT disposable resources. Both order-write flags and automatic deployment remain off. Phase 2/6 migrations already exist; never replay blindly.

WhatsApp is saved-order Open/Copy and manual customer Send only. No generic chat, accounts, payments, cart, notifications or Netlify. Vercel-only/free-only eligibility remains unresolved and production is held. Outstanding exact retention/cancellation/delivery facts were asked once; do not infer an answer. Carry them into P10, along with runtime grants, staff secrets, real publication and backup/full restore gates.

Continue P9.1 media-gap reconciliation, then responsive/motion/performance source refinement. Use current phase-8-content-proposals.json instead of treating the original historical publication candidate as the latest copy. No need for live credentials or a database operation to continue source work.
