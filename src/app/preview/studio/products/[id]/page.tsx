...
export default async function Page({params}:{params:Promise<{id:string}>}){await requirePublicPreview();const {id}=await params;if(!concepts.some(p=>p.id===id))notFound();return <ApprovedExperience initialRoute={`/studio/products/${id}`}/>;}
