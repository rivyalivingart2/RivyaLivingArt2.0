...
export async function GET(request:Request){
 try{
  ...
  const rows=await studioDb()`SELECT order_id AS id,last_contact_at AS "lastContactAt",became_order AS "becameOrder",closed_at AS "closedAt",ongoing_follow_up AS "ongoingFollowUp",hold_reason AS "holdReason",hold_review_on AS "holdReviewOn",deletion_requested_at AS "deletionRequestedAt",identity_verified_at AS "identityVerifiedAt",version FROM rivya_privacy_controls WHERE order_id=${id}::uuid`;
  if(!rows.length)return json({error:'Retention controls are unavailable for this record.'},404);
  return json({record:rows[0],decision:retentionDecision(rows[0] as RetentionRecord),deletionAvailable:false});
 }catch{return json({error:'Retention controls are temporarily unavailable. No deletion has been performed.'},503);}
}
export async function POST(request:Request){
 ...
 try{
  ...
  if(!b||!uuid(b.id)||!Number.isSafeInteger(b.version)||b.version<1||Object.keys(b).some(k=>!['id','version','lastContactAt','ongoingFollowUp','holdReason','holdReviewOn','recordDeletionRequest','verifyIdentity'].includes(k)))return json({error:'Invalid retention change.'},400);
 ...
 }catch{return json({error:'Retention changes could not be confirmed. Refresh before retrying. No deletion has been performed.'},503);}
}
