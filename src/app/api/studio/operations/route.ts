 ...
 try{
  ...
  if(view==='activity'){
   ...
   const edits=await sql`SELECT actor,action,entity,created_at AS "createdAt" FROM rivya_audit
    WHERE ${session.role==='admin'} OR actor=${session.adminId} ORDER BY created_at DESC LIMIT 100`;
...
export async function POST(request:Request){
 ...
 try{
  const session=await studioSession();if(!session)return json({error:'Sign in to continue.'},401);
  if(session.role!=='admin')return json({error:'Administrator access required.'},403);
  const body=await smallJson(request,1000);if(body.action!=='export'||body.confirm!=='private-export')return json({error:'Confirm the private export.'},400);
  const sql=studioDb();
  const rows=await sql`SELECT o.id,i.reference,o.client,o.title,o.status,i.phone,i.email,i.follow_up,o.created_at
    FROM rivya_studio_orders o LEFT JOIN rivya_inquiries i ON i.id=o.id ORDER BY o.created_at DESC LIMIT 5001`;
  ...
  const columns=['id','reference','client','title','status','phone','email','follow_up','created_at'];
  ...
  await sql`INSERT INTO rivya_audit(actor,action,entity) VALUES(${session.adminId},'orders:export',${String(rows.length)+' records'})`;
