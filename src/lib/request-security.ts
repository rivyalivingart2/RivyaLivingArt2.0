...
export async function clientRateKey(scope:string){const h=await headers();const ip=process.env.VERCEL?h.get('x-vercel-forwarded-for')?.split(',')[0]?.trim():process.env.NETLIFY?h.get('x-nf-client-connection-ip')?.trim():'local';return `${scope}:${privateHash(ip||'unknown')}`;}
