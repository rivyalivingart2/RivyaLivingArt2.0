...
export function validBusiness(value:unknown):value is BusinessSettings{
 ...
 if(typeof v.phone!=='string'||!/^\+[1-9]\d{9,14}$/.test(v.phone)||typeof v.whatsapp!=='string'||!/^\d{10,15}$/.test(v.whatsapp)||typeof v.email!=='string'||v.email.length>180||!/^\S+@[^\s@]+\.[^\s@]+$/.test(v.email)||typeof v.map!=='string'||v.map.length>500)return false;
