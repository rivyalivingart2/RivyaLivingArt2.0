...
export function StudioOverview({ data }: { data: StudioOverviewData }) {
  ...
  const metrics = [
    { label: "Product concepts", value: data.total, note: "Labelled source fixtures", href: "/studio/products" },
    { label: "Working drafts", value: data.drafts, note: "Demo draft status", href: "/studio/products?draft=DRAFT" },
    { label: "Awaiting review", value: data.inReview, note: "Demo review status", href: "/studio/products?draft=IN_REVIEW" },
    { label: "Visuals missing", value: data.pendingVisuals, note: "No approved image supplied", href: "/studio/products?media=VISUAL_PENDING" },
  ];

  return <div className={styles.page}>
    <header className={styles.heading}><div><p className={styles.eyebrow}>Workspace / Overview</p><h1>The atelier, at a glance.</h1><p>A clear view of the catalogue and the work still taking shape.</p></div><Link href="/studio/products/new" className={styles.primary}>Create local product draft <span aria-hidden="true">+</span></Link></header>
    ...
    {dataset === "live" ? <section className={styles.disconnected} aria-labelledby="live-data-heading"><span className={styles.emptyMark} aria-hidden="true">◇</span><p className={styles.eyebrow}>Live workspace / Not connected</p><h2 id="live-data-heading">Real work belongs to real data.</h2><p>No live catalogue, enquiries, staff session or business analytics are available in this frontend preview. Live and demo totals will stay separate when the backend is connected.</p><button className={styles.primary} type="button" onClick={() => setDataset("demo")}>Return to demo fixtures <span aria-hidden="true">↗</span></button></section> : <>
      ...
      <div className={styles.grid}>
        <section className={styles.panel} aria-labelledby="tier-breakdown"><header className={styles.panelHeading}><div><h2 id="tier-breakdown">One catalogue, three scales</h2><p>All {data.total} concepts are fictional demo records.</p></div><Link href="/studio/products">View catalogue <span aria-hidden="true">↗</span></Link></header>
          ...
          <div className={styles.tableWrap}><table className={styles.tierTable}><caption className="sr-only">Demo product totals, working drafts and missing visuals by tier</caption><thead><tr><th scope="col">Collection</th><th scope="col">Concepts</th><th scope="col">Drafts*</th><th scope="col">Missing visuals</th></tr></thead><tbody>{data.tiers.map((tier) => <tr key={tier.tier}><th scope="row"><Link href={`/studio/products?tier=${tier.tier}`}><i data-tier={tier.tier} aria-hidden="true" />{tier.label}</Link></th><td>{tier.count}</td><td>{tier.drafts}</td><td>{tier.pendingVisuals}</td></tr>)}</tbody></table></div>
        ...
        <section className={styles.panel} aria-labelledby="attention-heading"><header className={styles.panelHeading}><div><h2 id="attention-heading">A few next steps</h2><p>Selected source records needing attention.</p></div></header><div className={styles.attention}>{data.attention.length ? data.attention.map((item) => <Link key={item.id} href={`/studio/products/${item.id}`}><span className={styles.recordId}>{item.id}</span><span><strong>{item.title}</strong><small>{item.issue}</small></span><span aria-hidden="true">↗</span></Link>) : <p className={styles.footnote}>No matching sample records in this source snapshot.</p>}</div></section>
      </div>
      <section className={styles.panel} aria-labelledby="operations-heading"><header className={styles.panelHeading}><div><h2 id="operations-heading">Operations</h2><p>These services have no connected dataset in this preview.</p></div><span className={styles.badge}>Integration pending</span></header><div className={styles.operations}>{[
        { label: "Enquiry inbox", detail: "No saved enquiries or response backlog", href: "/studio/modules/inquiries" },
        { label: "Large-project pipeline", detail: "No persisted project or order stages", href: "/studio/modules/inquiries" },
        { label: "File imports", detail: "No import runs or failure history", href: "/studio/modules/imports" },
