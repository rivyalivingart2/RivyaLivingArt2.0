...
export function Gallery({product:p}:{product:Pick<ShopProduct,'image'|'imageAlt'|'imagePosition'|'imageCaption'|'scene'|'sceneAlt'|'scenePosition'|'sceneCaption'|'name'|'gallery'>}){
 ...
 <Image retry src={current.src} alt={current.alt} style={{objectPosition:current.position,objectFit:'contain'}} fill loading="eager" fetchPriority={active===0?'high':'auto'} sizes={imageSizes.detail}/>
