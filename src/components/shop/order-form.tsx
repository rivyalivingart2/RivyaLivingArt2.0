...
export function OrderForm(props:Props){
 ...
 useEffect(()=>{
  ...
  if(bespoke){setReady(false);return;}
  const controller=new AbortController();let active=true;
  setReady(false);setReconnecting(true);
  void fetch('/api/inquiry/session',{method:'POST',signal:controller.signal}).then(async r=>{const data=await r.json();if(!r.ok)throw Error(data.error||'The form is temporarily unavailable.');if(active){setReady(true);notify('Your secure form is ready.');}}).catch(e=>{if(active)notify(e instanceof Error?e.message:'Unable to prepare the form.',true);}).finally(()=>{if(active)setReconnecting(false);});
 ...
 function next(){const issues=step===0?validateAnswers(definition.fields,answers):contactErrors();setErrors(issues);if(Object.keys(issues).length){notify('Please check the linked fields below.',true);requestAnimationFrame(()=>heading.current?.focus());return;}notify('');go(step+1);}
 ...
 async function submit(){
  if(lock.current||!ready||bespoke||review.length||replacement)return;
  const issues={...validateAnswers(definition.fields,answers),...contactErrors()};setErrors(issues);
  if(Object.keys(issues).length){notify('Please check your details.',true);go(Object.keys(validateAnswers(definition.fields,answers)).length?0:1);return;}
  lock.current=true;setBusy(true);setSealed(true);notify('Saving your inquiry and references…');
  const form=new FormData();form.set('kind',definition.kind);form.set('schemaId',definition.schemaId);form.set('schemaVersion',String(definition.schemaVersion));form.set('productId',definition.product!.id);form.set('revision',String(definition.schemaVersion));form.set('requestKey',key);form.set('consent','on');form.set('website',honeypot.current?.value||'');
 ...
 {bespoke&&<p className={s.serviceNotice} role="status">Saving custom-piece requests is temporarily unavailable. You can prepare and copy your brief here. References cannot be uploaded yet, and nothing will be saved or sent.</p>}
 ...
 {([{id:'name',label:'Your name *',type:'text',auto:'name',max:100},{id:'phone',label:'Phone number *',type:'tel',auto:'tel',max:20},{id:'email',label:'Email (optional)',type:'email',auto:'email',max:180}] as const).map(f=><label className={s.field} key={f.id} htmlFor={'contact-'+f.id}>{f.label}<input id={'contact-'+f.id} type={f.type} required={f.id!=='email'} autoComplete={f.auto} maxLength={f.max} value={contact[f.id]} onChange={e=>setContact(v=>({...v,[f.id]:e.target.value}))} aria-invalid={!!errors[f.id]} aria-describedby={'error-'+f.id}/><span id={'error-'+f.id} className={s.error}>{errors[f.id]}</span></label>)}
 <label className={s.field+' '+s.wide} htmlFor="contact-notes">Notes (optional)<textarea id="contact-notes" aria-invalid={!!errors.notes} aria-describedby="error-notes" rows={4} maxLength={1200} value={contact.notes} onChange={e=>setContact(v=>({...v,notes:e.target.value}))}/><span id="error-notes" className={s.error}>{errors.notes}</span></label>
 <label id="contact-references" tabIndex={-1} className={s.field+' '+s.wide} htmlFor="reference-files">Reference images (optional)<input id="reference-files" type="file" multiple accept="image/jpeg,image/png,image/webp" disabled={!ready||transferring||refs.length===3||bespoke} aria-invalid={!!errors.references} aria-describedby="reference-help reference-error" onChange={e=>{const files=Array.from(e.target.files||[]);e.target.value='';void select(files);}}/><span id="reference-help" className={s.help}>Up to 3 images, 3 MB each (9 MB total). Stored privately for authorized staff. Images cannot attach automatically to a WhatsApp text link.</span></label>
 </div><div aria-live="polite" aria-relevant="additions text">{refs.map(r=><div className={s.uploadItem} key={r.localId}><span>{r.file.name} · {r.state==='saved'?'Saved privately':r.state==='uploading'?(r.progress===100?'Processing securely…':'Uploading…'):r.state==='failed'?r.error:'Waiting…'}{r.state==='uploading'&&<progress max={100} value={r.progress||0} aria-label={'Upload progress for '+r.file.name}/>}</span><div>{r.state==='failed'&&<button type="button" disabled={!ready||transferring} onClick={()=>void runUploads([r])}>Retry</button>}<button type="button" disabled={transferring} onClick={()=>void remove(r)}>Remove</button></div></div>)}</div><p id="reference-error" className={s.error}>{errors.references}</p>
 <label className={s.check}><input id="contact-consent" type="checkbox" required aria-invalid={!!errors.consent} aria-describedby="error-consent" checked={consent} onChange={e=>setConsent(e.target.checked)}/><span>I agree to saving my brief, contact details and references for this order request. The WhatsApp link will contain my contact details and text brief. I choose whether to send it. <Link className={s.textLink} href="/privacy" target="_blank" rel="noreferrer">Privacy details (opens a new tab)</Link></span></label><p id="error-consent" className={s.error}>{errors.consent}</p></fieldset>}
 ...
 {!bespoke&&!ready&&<button type="button" className={s.button} disabled={busy||reconnecting} onClick={()=>setSessionAttempt(v=>v+1)}>{reconnecting?'Connecting…':'Reconnect form'}</button>}
 {!bespoke&&<button type="button" className={s.textLink} disabled={editingLocked||refreshing||transferring} onClick={()=>void refreshSchema()}>{refreshing?'Checking options…':'Check current options'}</button>}
 <div className={s.actions}>{step>0&&<button type="button" className={s.button+' '+s.outline} disabled={editingLocked} onClick={()=>go(step-1)}>← Back</button>}<button type="submit" className={s.button} disabled={busy||transferring||!!review.length||!!replacement||(step===2&&(!ready||bespoke))}>{busy?'Saving inquiry…':step===2?(bespoke?'Saving temporarily unavailable':sealed?'Retry the same save':'Save order details →'):step===0?'Your details →':'Review your brief →'}</button></div>
