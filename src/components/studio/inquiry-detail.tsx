...
export function InquiryDetail({initial,staff,admin,onSaved}:{initial:InquiryDetailData;staff:StaffMember[];admin:boolean;onSaved:()=>Promise<void>}){
 ...
 const dirty=assignmentDirty||!!note||!!reason||!!instructions;
 ...
 return <div className={s.inquiryDetail} data-unsaved={dirty} data-studio-saving={busy}>
  ...
  <section className={s.panel}><h3>Original customer submission</h3><p>This is the saved brief. Staff changes below never replace these answers or the original order message.</p><dl>
   ...
   <dt>Name</dt><dd>{i.name}</dd><dt>Phone</dt><dd><a href={'tel:'+i.phone}>{i.phone}</a></dd><dt>Email</dt><dd>{i.email||'Not provided'}</dd><dt>Received</dt><dd>{new Date(i.createdAt).toLocaleString()}</dd>
  ...
  <section className={s.panel}><h3>Operational amendments</h3><p>Record agreed changes with a reason. Amendments are append-only staff instructions; the customer's original brief and saved WhatsApp message stay unchanged.</p>
   {data.amendments.length?data.amendments.map(a=><article key={a.id} className={s.notes}><small>{a.actor} · {new Date(a.createdAt).toLocaleString()} · Record version {a.orderVersion}</small><p><strong>Reason:</strong> {a.reason}</p><p>{a.instructions}</p></article>):<p>No amendments recorded.</p>}
  ...
  <section className={s.panel}><h3>Internal notes</h3>{data.notes.length?data.notes.map(n=><article className={s.notes} key={n.id}><small>{n.actor} · {new Date(n.createdAt).toLocaleString()}</small><p>{n.body}</p></article>):<p>No internal notes yet.</p>}<form onSubmit={event=>{event.preventDefault();void save('note');}}><fieldset disabled={busy||conflict}><label>Add a private note<textarea rows={3} maxLength={2000} required value={note} onChange={event=>setNote(event.target.value)}/></label><button disabled={!note.trim()}>Save note</button></fieldset></form></section>
  <section className={s.panel}><h3>Original saved order message</h3><SavedOrderActions key={i.id} receipt={i.receipt} endpoint={'/api/studio/inquiry-handoff?id='+encodeURIComponent(i.id)} identity={{id:i.id}} onChange={receipt=>setData(v=>({...v,inquiry:{...v.inquiry,receipt}}))}/>{i.receipt.summary&&<pre className={s.summary}>{i.receipt.summary}</pre>}</section>
  <details className={s.panel}><summary>Stage history — {data.events.length} events</summary>{data.events.map((v,index)=><p key={index}>{new Date(v.createdAt).toLocaleString()} · {v.actor}: {v.from_status?stageLabel(v.from_status):'Received'} → {stageLabel(v.to_status)}{v.reason?' · '+v.reason:''}</p>)}</details>
 </div>;
}
