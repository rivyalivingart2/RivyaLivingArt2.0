...
export function StudioLogin({configured, unavailable}: {configured: boolean; unavailable: boolean}) {
  ...
  return <main id="main-content" tabIndex={-1} className="studio-login"><form action={action} className="studio-login-card" aria-busy={pending}>
    ...
    {!configured ? <p role="status">Studio setup is pending. The owner must finish configuration before sign-in is available.</p> : <>
      <label htmlFor="admin-id">Staff ID</label><input id="admin-id" name="id" autoComplete="username" maxLength={200} required autoFocus disabled={pending}/>
      ...
      <button type="button" className="studio-password-toggle" aria-pressed={visible} aria-controls="admin-password" disabled={pending} onClick={() => setVisible(v => !v)}>{visible ? 'Hide password' : 'Show password'}</button>
      <p className="studio-login-error" role="alert">{state.error || (unavailable ? 'Sign-in is temporarily unavailable. Please try again later.' : '')}</p>
      <button className="studio-button primary" type="submit" disabled={pending}>{pending ? 'Signing in…' : 'Sign in'}</button>
    </>}
    <p className="studio-note">Access is reserved for authorized atelier staff.</p>
  </form></main>;
}
