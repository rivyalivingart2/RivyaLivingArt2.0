# Active checkpoint — shared data configured; production release paused

The owner explicitly chose one shared live database, approved shared private reference storage, and then chose “Keep free-only; pause production” after the Hobby eligibility check. Read docs/decisions/2026-09-23-shared-database-production-hold.md and docs/redesign/SHARED-DATA-CONFIGURATION.md. The canonical master is docs/redesign/RivyaLivingArt-Commercial-Implementation-Plan.md revision 3.7. These decisions supersede earlier deployment authorization and application Preview isolation requirements.

Production database variables now point to the existing modern Preview database for FUTURE deployments; both environments have access to the same private reference store and RIVYA_DATA_MODE=shared. Existing deployed versions retain their captured configuration. No new deployment, database migration, data write, seed or content publication occurred. Both original databases and integration resource associations remain. Independent encrypted backups of both databases and rollback configuration exist outside the repository/provider. Recheck variable destinations after any integration reconnect/rotation; connection cards may show historical associations.

Phase 6 source and its additive schema are complete; never replay those migrations blindly. Both order write flags remain off. Synthetic QA, fixture imports and restore rehearsals require a DIFFERENT disposable database/private store. The import/bootstrap scripts require explicit isolated mode, and shared-data mode disables fixture presentation routes. Ordinary authorized Preview Studio edits will affect shared records once activated. Staff authorization remains required.

Next is P7.1: complete Studio detail and immutable original versus operational amendment separation. Phase 7 has not been executed. Main includes Phase 6 through PR #20 at 8fd2cf02174ccb81c67586389d5bc958fa442b29. Configuration safeguards and checkpoint continue on codex/shared-database-configuration. Verify current refs; preserve all drafts, records, IDs, assets and private references.

WhatsApp is only saved-order Open/Copy with manual customer Send. No generic WhatsApp, payments, cart, customer accounts or Netlify. Keep vercel.json automatic deployment disabled. No upgrade/trial/deployment while the owner's free-only production hold remains. Later gates include runtime grants, environment-specific staff secrets, reviewed real catalogue/content/settings, privacy/retention, portable backups/full isolated restore and Phase 11 application QA. No repeat plan, shared-database or private-storage approval is needed.
